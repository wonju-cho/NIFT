...
public class GiftHistoryServiceImpl implements GiftHistoryService {
    ...
    public void sendGiftHistory(Long senderId, SendGiftDto request) {
        GifticonNFT.GiftPendingEventResponse giftPendingEventResponse = transactionService.getGiftPendingEventByTxHash(request.getTxHash()).getFirst();
        log.info("[GiftHistoryService] giftPendingEventResponse: {}", giftPendingEventResponse);

        User sender = userRepository.findByWalletAddress(giftPendingEventResponse.sender).orElseThrow(() -> new GiftHistoryException(GiftHistoryErrorCode.CANNOT_FIND_BY_ADDRESS));
    ...
    private void giftFromArticle(Long senderId, SendGiftDto request, GifticonNFT.GiftPendingEventResponse giftPendingEventResponse) {
        ...
        contractService.addArticleHistory(article.getArticleId(), request.getTxHash(), senderId);
    }
}
