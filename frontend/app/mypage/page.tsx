      ...
      <a href="#" className="block">
        <div className="relative aspect-square overflow-hidden">
          ...
          >
            <svg
              ...
              className={`h-4 w-4 ${isLiked ? "fill-primary text-primary" : "text-gray-500"}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <div className="p-3">
          ...
          <div className="mt-2 flex items-baseline gap-1">
            ...
            {!!discountRate && discountRate > 0 && <span className="text-xs text-primary">{discountRate}% 할인</span>}          </div>
          {(sender || date) && (
            <div className="mt-2 text-xs text-gray-500">
              {sender && <div>from. {sender}</div>}
              {date && <div>{usedDate ? `사용일: ${usedDate}` : `받은날: ${date}`}</div>}
            </div>
          )}
        </div>
      </a>
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        setUser({
          profileImage: data.profileImage ? data.profileImage : "/placeholder.svg",
                    ...
                    <CardContent className="p-0">
                      <nav className="flex flex-col">
                        {sidebarItems.map((item) => (
                          ...
                          >
                            <item.icon className={`h-5 w-5 ${activeTab === item.value ? "text-primary" : ""}`} />
                  ...
                  <Card className="border-none shadow-md">
                    <CardContent className="p-6">
                      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        ...
                        >
                          <div>
                            <h2 className="mb-4 text-xl font-semibold">보유 NIFT</h2>
                          ...
                          >
                            ...
                            <TabsContent value="available" className="mt-6">
                              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {availableGiftCards
                                  .slice(availableCurrentPage * 6, (availableCurrentPage + 1) * 6)
                                  .map((card) => (
                                    <GiftCard
                                      key={card.serialNum}
                              ...
                              {availableGiftCards.length === 0 ? (
                                <div className="mt-8 mb-12 text-center text-muted-foreground">사용 가능한 선물이 없습니다.</div>
                              ) : (
                                <div className="mt-8 flex justify-center items-center gap-2">
                                  <Button
                                    ...
                                    onClick={() => setAvailableCurrentPage((prev) => Math.max(prev - 1, 0))}
                                  >
                                    ‹ 이전
                                  </Button>
                                  ...
                                  {(() => {
                                    ...
                                    return Array.from({ length: end - start }, (_, i) => i + start).map((pageNum) => (
                                      <Button
                                        key={pageNum}
                                        variant={current === pageNum ? "default" : "ghost"}
                                        size="sm"
                                        onClick={() => setAvailableCurrentPage(pageNum)}
                                      >
                                        {pageNum + 1}
                                      </Button>
                                    ));
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    disabled={availableCurrentPage === availableTotalPage - 1}
                                    onClick={() =>
                                      setAvailableCurrentPage((prev) => Math.min(prev + 1, availableTotalPage - 1))
                                    }
                                  >
                                    다음 ›
                                  </Button>
                            ...
                            <TabsContent value="used" className="mt-6">
                              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {usedGiftCards
                                  .slice(usedCurrentPage * 6, (usedCurrentPage + 1) * 6)
                                  .map((card) => (
                                    <GiftCard
                                      key={card.serialNum}
                                      serialNum={card.serialNum}
                              ...
                              {usedGiftCards.length === 0 ? (
                                <div className="mt-8 mb-12 text-center text-muted-foreground">사용 완료된 선물이 없습니다.</div>
                              ) : (
                                <div className="mt-8 flex justify-center items-center gap-2">
                                  <Button
                                    variant="ghost"
                                    ...
                                    onClick={() => setUsedCurrentPage((prev) => Math.max(prev - 1, 0))}
                                  >
                                    ‹ 이전
                                  </Button>
                                  ...
                                  {(() => {
                                    ...
                                    return Array.from({ length: end - start }, (_, i) => i + start).map((pageNum) => (
                                      <Button
                                        ...
                                        variant={current === pageNum ? "default" : "ghost"}
                                        size="sm"
                                        onClick={() => setUsedCurrentPage(pageNum)}
                                      >
                                        {pageNum + 1}
                                      </Button>
                                    ));
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    disabled={usedCurrentPage === usedTotalPage - 1}
                                    onClick={() =>
                                      setUsedCurrentPage((prev) => Math.min(prev + 1, usedTotalPage - 1))
                                    }
                                  >
                                    다음 ›
                                  </Button>
                        ...
                        <TabsContent value="memories" className="mt-6">
                          <div>
                            <h2 className="mb-4 text-xl font-semibold">선물 추억</h2>
                            <p className="mb-6 text-sm text-muted-foreground">
                          ...
                          <div className="space-y-6">
                            ...
                            <Card>
                              <CardHeader>
                                ...
                                <CardDescription>계정 관련 중요 설정을 관리합니다.</CardDescription>
                              </CardHeader>
                              <CardContent className="space-y-4">
                                ...
                                <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
                                  ...
                                  <DialogContent>
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        회원 탈퇴 시 모든 계정 정보와 거래 내역이 영구적으로 삭제됩니다. 이 작업은
                                        되돌릴 수 없습니다.
                                      </DialogDescription>
                                    </DialogHeader>
                                    <div className="space-y-4 py-4">
                                      <div className="space-y-2">
                                        ...
                                        <Input id="confirm" placeholder="탈퇴합니다" />
                                      </div>
                                    </div>
                                    <DialogFooter>
